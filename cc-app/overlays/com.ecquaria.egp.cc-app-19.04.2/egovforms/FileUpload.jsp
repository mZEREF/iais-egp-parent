...
<link href="<%=EngineHelper.getResourcePath()%>/_statics/css/fileUpload.css" rel="stylesheet" type="text/css" media="all" />
...
<div <% if(elementID != null) { out.print("id=" + elementID);} %> class="pop-up upload <%= classNames %>">
    <form action="<%= action%>" name="fileUploadForm" method="post">
        ...
        <div class="pop-up-body">
            ...
            <div class="content-body fileUploadContainer">
            	<%
            		...
            		String mf = "(Only support file type: "+doc.getFileType()+")";
            		String asterisk = "";
            		if(isRequired){
            	...
            	%>
                <div class="field"><label><%= doc.getName() %> <%= desc %>:</label></div>
                ...
                <div class="control">
					...
					<% if(canUpload) {%>
					<a id="browseFile-<%=doc.getId()%>" class="browseFile" title="<%= doc.getFileType() %>">Browse Files</a>  <%=mf %>
					<% }%>                    
					<div class="fileList">
                        <%
                            if(docUploads != null) {
		...
		<% if(canDelete) {%>
			<a href="javascript:void(0);" class="removeServerFile" title="Delete"><img src="<%=request.getContextPath() %>/_statics/egov-fileupload/images/delete-file.png"></a>
		<% }%>                    		
            ...
            <div style="padding-bottom:10px;" class="content-body fileUploadContainer">
                <div class="field"><label>Upload your files:</label></div>
                <span class="fileType" style="display:none"></span>
                <span class="fileMandatory" style="display:none">No</span>
                <div class="control">
					<% if(canUpload) {%>
						<a id="browseFile"class="browseFile">Browse Files</a><br />
					<% }%>                    
        ...
        <div class="action-button" style="display:none;">
		    <%
		        ...
		        if(buttons!=null){
		        	...
		        	out.print(new FileUploadButton(FileUploadUtil.BUTTON_SUBMIT_DEFAULT_LABEL, "upload").generateButton());
		        }
		    %>
		    
		    <% if(embedMode && canUpload) {%>
				<a class=" red uploadAll"><%= buttonText %></a>		    
			<% } %>
		</div>
    </form>
</div>
...
<script type="text/javascript">
    ...
    $(function() {
            ...
            uploader.bind('FilesAdded', function(up, files) {
            	...
            	$.each(up.files, function (i, file) {
            		if(up.files.length > 0 && fileList.find('.server-site').length>0){
            			SOP.Common.alert("<egov-smc:message key='needremovefirst'>You need to remove the file from server first.</egov-smc:message>");
            			up.removeFile(file);
            			return;
            		}
        ...
        $(".removeServerFile").live("click",function() {
            ...
            var fileId = $(this).parent().attr("id");
            var $me = $(this);
			SOP.Common.confirm({"message": '<egov-smc:message key="confirmremove">Are you sure you want to remove the file from server?</egov-smc:message>', func: function(){
    	...
    	if(options.checkMandatory){
	    	for(var i=0; i<uploaders.length; i++) {
	    		...
	    		if(uploaders[i].settings.mandatory && (uploaders[i].files.length == 0 && fileList.find('.server-site').length == 0)){
	    			...
	    			needNames += (needNames?",":"") + uploaders[i].settings.multipart_params.file_type;
	    		}
	    	}
	    	if(needFile){
	    		SOP.Common.alert("<egov-smc:message key='plsupload'>Please upload the mandatory document(s):</egov-smc:message><br>"+needNames);
	    		SOP.Common.hideMask();
	    		return;
	    	}
    	}
