...
<Control type="${control.FQN}" ref="${control.properties.name}"
	...
	<Properties>
		...
		{if control.properties.selected}
		<Entry name="selected"><String>${control.properties.selected}</String></Entry>
		{/if}
		{if control.properties.empty_option !== undefined}
			<Entry name="empty_option"><String>${control.properties.empty_option|escape}</String></Entry>
			{if control.properties.empty_option === 'true' && control.properties.empty_option_desc !== undefined}
				<Entry name="empty_option_desc"><String>${control.properties.empty_option_desc|escape}</String></Entry>
			{/if}
		{/if}
		...
		<Entry name="conditionModelPersistentMap">
			<Map>
				{for row in condition.persistentMap}
					<Entry name="${row.key}"><String>${row.val}</String></Entry>
				{/for}
			</Map>
		</Entry>
