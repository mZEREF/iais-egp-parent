...
public class SendEmailThread extends Thread {
    ...
    private String applicaitonGroupId ;
    ...
    public SendEmailThread(String templateId, String applicaitonGroupId, Map<String, Object> templateContent, String mailSender,String queryCode,String reqRefNum){
        this.applicaitonGroupId = applicaitonGroupId;
    ...
    public void run(){
        try
        {
            ...
            log.info(StringUtil.changeForLog("sendemail start... application is"+this.applicaitonGroupId + "templateId is "+ this.templateId+"thread name is " + Thread.currentThread().getName()));
            ...
            if(msgTemplateDto.getRecipient()!= null && msgTemplateDto.getRecipient().size() > 0){
                receiptemail = emailHelper.getRecript(msgTemplateDto.getRecipient(),this.applicaitonGroupId);
               emailDto.setReceipts(receiptemail);
            }
            if(msgTemplateDto.getCcrecipient()!= null && msgTemplateDto.getCcrecipient().size() > 0){
                ccemail = emailHelper.getRecript(msgTemplateDto.getCcrecipient(),this.applicaitonGroupId);
                emailDto.setCcList(ccemail);
            }
            if(msgTemplateDto.getBccrecipient()!= null && msgTemplateDto.getBccrecipient().size() > 0){
                bccemail = emailHelper.getRecript(msgTemplateDto.getBccrecipient(),this.applicaitonGroupId);
