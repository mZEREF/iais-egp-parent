...
<%@page
        import="com.ecquaria.egp.core.helper.EGPCommonHelper"%>
<%@page import="com.ecquaria.egp.core.helper.ServiceRegistryHelper"%>
...
<%@page import="sop.i18n.TranslationConstants"%>
<%@page import="sop.usergroup.UserGroup"%>
<%@page import="sop.web.smc.SmcUtil"%>
...
<%
    ...
    boolean endorsementOfficer = (boolean) request.getAttribute("endorsementOfficer");
    pageContext.setAttribute("endorsementOfficer", endorsementOfficer);
    EndorsementTask endorEntity = (EndorsementTask) request.getAttribute("endorsementTask");
    String currentStatus = endorEntity.getEndorseStatus();
    ...
    viewFormUrl = RedirectUtil.changeUrlToCsrfUrl(viewFormUrl, request);
%>
...
<script type="text/javascript">
    ...
    function updateStatus(){
        ...
        var currentSt = '<%=displayCurrentStatus%>';
        var updateSt = $("#selectedStatus").find("option:selected").text();
        var messageStr = "Application No.:"+appNo +"<br>"+"Current Status:"+currentSt+"<br>"+"Update Status:"+updateSt+"<br>"+'<egov-smc:messageTemplate key="egp.application.view.update.confirm" default="Are you sure you want to update status?" />';
        SOP.Common.confirm({
            message: messageStr,
            func: function(){
                performAction(action);
...
<webui:setAttribute name="title">
    <egov-smc:titleLabel id="oriTitle">
        <c:choose>
            <c:when test="${endorsementOfficer}">
                <c:out value="Endorsement Details"/>
            </c:when>
            <c:otherwise>
                <c:out value="Application Details"/>
            </c:otherwise>
        </c:choose>
    </egov-smc:titleLabel>
    <c:out value="${oriTitle }"/>
</webui:setAttribute>

<%--<egov-core:breadcrumb>
	<egov-core:breadcrumbEntry label="Home" url="/process/SYSTEM/Home" />
	&lt;%&ndash;<egov-core:breadcrumbEntry label="Applications"
		url="/process/EGOV/ApplicationList" />&ndash;%&gt;
	<egov-core:breadcrumbEntry label="Applications"
							   url="/process/EGPCLOUD/ApplicationList" />
...
<c:choose>
    <c:when test="${endorsementOfficer}">
        <egov-core:breadcrumb>
            ...
            <egov-core:breadcrumbEntry label="Endorsements" url="../egov/process/EGPCLOUD/EndorsementList"/>
            <egov-core:breadcrumbEntry label="Endorsement Details"></egov-core:breadcrumbEntry>
        </egov-core:breadcrumb>
        <h1>
            <egov-smc:commonLabel>
                <c:out value="Endorsement Details" />
            </egov-smc:commonLabel>
        </h1>
    </c:when>
...
<div class="onecolumn">
    <div id="rotate">
        <div class="header">
            ...
            <c:if test="${needHistory && !endorsementOfficer}">
                <img style="float: right; margin-top: 10px; cursor: pointer;"
                    ...
                    <div class="inner-accordion" id="innerAccordiion1">
                        ...
                        <div class="content">
                            <input type="hidden"
                                   ...
                                   name="entityUids" />
                            <input type="hidden"
                                   value='<c:out value="${taskEntity.id}"></c:out>'
                                   name="taskid">
                                ...
                                <%	Date dateCreated = null;
                                    ...
                                    if(task!=null && slaInDays != 0){
                                        if(!task.isCompleted()){
                                            ...
                                            if(remainCount >= 0) {
                                                countStr = SmcUtil.getTranslatedValue(pageContext.getSession(), TranslationConstants.SOP_SMC_LABEL, MSG_COUNT_LEFT, new String[]{String.valueOf(remainCount)} );
                                            } else {
                                                countStr = SmcUtil.getTranslatedValue(pageContext.getSession(), TranslationConstants.SOP_SMC_LABEL, MSG_COUNT_OVERDUE, new String[]{String.valueOf(-remainCount)} );
                                            }
                    ...
                    <div class="inner-accordion" id="innerAccordiion4">
                        ...
                        <div class="content">
                           ...
                           <egov-core:ifPrivilegeAccessible privilegeId="<%=RbacConstatns.EGOV_USER_HANDLE_INTERNAL_DOC_PRIVILEGE%>" var="isUserCanHandleInternal"/>--%>
                            <p class="text">
                                <egov-smc:commonLabel>These are documents uploaded by the applicant or an officer on behalf of the applicant. Listed documents are those defined for this e-service only.</egov-smc:commonLabel>
                            </p>
                            <table class="table2 view-table" style="border: 1px; border-style:solid;border-color:#d0d0d0;">
                                ...
                                <tr style="border:0px;background-color: #A5A5A5;">
                                    ...
                                    <td class="th" align="left" width="20%">
                                        <div class="sort-label">Date Submitted</div></td>
                                ...
                                <%
                                    ...
                                    if(dss != null && dss.size()>0){
                                        for(int i = 0;i<dss.size();i++){
                                            ...
                                            if(doc==null ){
                                                actionType = "<a  style=\"cursor: pointer;\" onclick=\"openSupportDoc('"+docName+"','"+ds.getFileType()+"');\" href=\"javascript:void(0);\" title=\"Upload\"><img alt=\"Upload\" src=\"/web/_themes/egov/images/general/arrow-090.png\"/></a>"+"&nbsp;&nbsp;&nbsp;&nbsp;"+
                                                        "<a style=\"cursor: pointer;\" onclick=\"openSupportDocHistory('"+displayDocName+"','"+batchId+"','"+request.getContextPath()+"','"+appNo+"','"+docName+"');\" href=\"javascript:void(0);\" title=\"History\"><img alt=\"History\" src=\"/web/_themes/egov/images/general/view.png\"/></a>";
                                            }else{
//									if(UserDomainHelper.isCounterStaffDomain(doc.getCreatedUserDomain())){
                                                actionType = "<a  style=\"cursor: pointer;width:16px\" onclick=\"deleteSupportDoc('"+doc.getDocId()+"');\" href=\"javascript:void(0);\" title=\"Remove\"><img alt=\"Remove\" src=\"/web/_themes/egov/images/general/delete.png\"/></a>"+"&nbsp;&nbsp;&nbsp;&nbsp;"+
                                                        "<a  style=\"cursor: pointer;\" onclick=\"openUpdateSupportDoc('"+doc.getDocId()+"','"+docName+"','"+ds.getFileType()+"');\" href=\"javascript:void(0);\" title=\"Upadte\"><img alt=\"Update\" src=\"/web/_themes/egov/images/general/edit.png\"/></a>"+"&nbsp;&nbsp;&nbsp;&nbsp;"+
                                                        "<a style=\"cursor: pointer;\" onclick=\"openSupportDocHistory('"+displayDocName+"','"+batchId+"','"+request.getContextPath()+"','"+appNo+"','"+docName+"');\" href=\"javascript:void(0);\" title=\"History\"><img alt=\"History\" src=\"/web/_themes/egov/images/general/view.png\"/></a>";
//									}
                                            }
                                %>
                                <tr style="width:50%">
                                    ...
                                    <td style="border: 1px; border-style:solid; text-align: left;width: 16%;border-color:#d0d0d0;">
                                        <c:if test="${isUserCanUploadDoc == true or isUserCanUpdateStatus == true}">
                                            <c:if test="${isUserCanHandleSupport && !endorsementOfficer}">
                                                <egov-smc:commonLabel><%=actionType%></egov-smc:commonLabel>
                                            </c:if>
                                        </c:if>
                                    </td>
                                </tr>
                    ...
                    <div class="inner-accordion" id="innerAccordiion5">
                        ...
                        <div class="content">
                            <p class="text">
                                <egov-smc:commonLabel>These are documents uploaded by an agency officer to support back office processing.</egov-smc:commonLabel>
                            </p>
                            <script type="text/javascript">
                            ...
                            <table class="table2 view-table" style="border: 1px; border-style:solid;border-color:#d0d0d0;">
                                ...
                                <tr style="border:0px;background-color: #A5A5A5;">
                                    ...
                                    <c:if test="${isUserCanUploadDoc == true or isUserCanUpdateStatus == true}">
                                        <c:if test="${isUserCanHandleInternal && !endorsementOfficer}">
                                ...
                                <tr style="width:50%">
                                    ...
                                    <td  style="border: 1px; border-style:solid; text-align: left;width: 16%;border-color:#d0d0d0;"><a href="<%=request.getContextPath()%>/process/EGOV/DownloadInternalDocument?docId=<%=doc.getDocId()%>"><%=doc.getDocFilename() %></a></td>
                                    <td  style="border: 1px; border-style:solid; text-align: left;width: 16%;border-color:#d0d0d0;"><%=s %></td>
                                    <td  style="border: 1px; border-style:solid; text-align: left;width: 16%;border-color:#d0d0d0;"><%=doc.getUpdatedUserId() %></td>
                                    <td  style="border: 1px; border-style:solid; text-align: left;width: 16%;border-color:#d0d0d0;"><%=uploadDate %></td>
                                    <c:if test="${isUserCanUploadDoc == true or isUserCanUpdateStatus == true}">
                                        <c:if test="${isUserCanHandleInternal && !endorsementOfficer}">
                                            <td  style="border: 1px; border-style:solid; text-align: left;width: 16%;border-color:#d0d0d0;"><a href="javascript:void(0);" onclick="deleteInternalDoc('<%=doc.getDocId() %>');" style="cursor: pointer;">Remove</a></td>
                                        </c:if>
                                    </c:if>
                                </tr>
                                <script type="text/javascript">
                            ...
                            <%
                                ...
                                if(isUserCanUploadDoc || isUserIsCurrent){
                            %>
                            <c:if test="${isUserCanHandleInternal && !endorsementOfficer}">
                                <div class="action-buttons" align="right">
                                    <button type="button"  style="width:150px;" onclick="openUploadDoc();">Upload Document</button>
                                </div>
                            </c:if>
                            <%
                                }
                    ...
                    <div class="inner-accordion" id="innerAccordiion6">
                        ...
                        <div class="content">
                            <table class="table2 view-table"
                                ...
                                <tr style="width: 50%">
                                    <td
                                            style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=payment.getPaymentDesc() %></td>
                                    <td
                                    ...
                                    <td
                                            style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=payment.getPaymentStatus() %></td>
                                    <td
                                            style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=payment.getTxRefNo() %></td>
                                    <td
                                            style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=payment.getPaymentType()%></td>
                                </tr>
                                <%
                                    }
                                }else{
                                ...
                                <tr>
                                    ...
                                    <td align="left"
                                        ...
                                        <select id="selectedStatus" name="selectedStatus">
                                            ...
                                            <option value="110001001" selected="selected">Approved
                                            </option>

                                            <option value="110001002">Rejected
                                            </option>
                                                    ...
                                                    <%
                                                        ...
                                                        if (endorsementHistories != null && endorsementHistories.size() > 0) {
                                                            for (EndorsementTask endorsementTask : endorsementHistories) {
                                                                String officer = endorsementTask.getOfficerId();
                                                                if (StringHelper.isEmpty(officer)) {
                                                                    officer = "-";
                                                                }
                                                                long workingGroupNo = endorsementTask.getWorkingGroup();
                                                                UserGroup endorseGroup = UserService.getInstance().getGroupsByGroupNo(workingGroupNo);
                                                                String workingGroupName = "-";
                                                                if(endorseGroup != null){
                                                                    workingGroupName = endorseGroup.getGroupFullName();
                                                                }
                                                                String displayStatus = endorsementTask.getEndorseStatus();
                                                                if (StringHelper.equals(displayStatus, EndorsementTask.STATUS_APPROVE)) {
                                                                    displayStatus = "Approved";

                                                                } else if (StringHelper.equals(displayStatus, EndorsementTask.STATUS_REJECT)) {
                                                                    displayStatus = "Rejected";

                                                                } else if (StringHelper.equals(displayStatus, EndorsementTask.STATUS_PENDING)) {
                                                                    displayStatus = "Pending endorsement";

                                                                }
                                                                String internalComments = endorsementTask.getInternalComments();
                                                                if (StringHelper.isEmpty(internalComments)) {
                                                                    internalComments = "-";
                                                                }
                                                                Date endorseDate = endorsementTask.getDateUpdated();
                                                                String hisDate = "-";
                                                                if (endorseDate != null) {
                                                                    hisDate = ConsistencyHelper.formatDateTime(endorseDate);
                                                                }
                                                    %>
                                                    <tr style="width: 50%">
                                                        <td style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=officer%></td>
                                                        <td style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=workingGroupName %></td>
                                                        <td style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=displayStatus%></td>
                                                        <td style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=internalComments%></td>
                                                        <td style="border: 1px; border-style: solid; text-align: left; border-color: #d0d0d0;"><%=hisDate%></td>
                                                    </tr>
                                                    <%
    ...
    <div id="last_content" class="batContent">
        ...
        <div id="msgTab-0" style="<c:out value='${cssEmail }'/>">
            <form id="emailInputForm">
                    ...
                    var updateMessage = function(panel){
                        var _sHandler = function(data, textStatus){
                            ...
                            for(var i=0;i<$data.length;i++){
                                var $tr = $('<tr><td>'+$data[i].dateSent+'</td><td>'+$data[i].officer+'</td><td>'+$data[i].type+'</td><td>'+ Base64.decode($data[i].content) +'</td></tr>');
                ...
                <div class="text-normal">
                    <label><%-- <egov-smc:fieldLabel name="To" /> --%>To</label> <span
                ...
                <div class="text-normal">
                    <label><%-- <egov-smc:fieldLabel name="Title" /> --%>Title</label>
                ...
                <div class="text-normal">
                    <label><%-- <egov-smc:fieldLabel name="Content" /> --%>Content</label>
            ...
            <script>
                ...
                var resetInputForm = function(formId){
                    SOP.Common.confirm({message:'<egov-core:escapeJavaScript value="${resetFormMsg}" />', func:function(){$('#'+formId).trigger('reset'); $('#' + formId + ' #smsChars').html(LIMIT_CHARS);}});
                }
            </script>
            <!--/begin btn-wrap  -->
        ...
        <div id="msgTab-1" style="<c:out value='${cssSMS }'/>">
            <form id="smsInputForm">
                ...
                <div class="text-normal">
                    <label><%-- <egov-smc:fieldLabel name="Content" /> --%>Content</label> <span
            ...
            <div class="history-content">
                <table width="100%" cellspacing="0"
                    ...
                    <c:forEach items="${appMessages }" var="msg">
                        ...
                        %>
                        <tr>
                            <td><fmt:formatDate value="${msg.createdDate }"
    ...
    <div id="confirm_content">
        <script type="text/javascript">
            var sendEmail2 = function(){
                ...
                if(!subject){
                    $('[name="emailSubject2"]').find('small.error').html('<egov-smc:messageTemplate key="<%=MessageTemplateConstants.EGP_MSG_COMMON_REQUIRED_VALUE%>" default="Required Value" />');
                    hasError = true;
                }

                if(!content){
                    $('[name="emailContent2"]').parents('.text').find('small.error').html('<egov-smc:messageTemplate key="<%=MessageTemplateConstants.EGP_MSG_COMMON_REQUIRED_VALUE%>" default="Required Value" />');
                    hasError = true;
                }
        ...
        <div class="batContent">
            ...
            <div id="msgTab2-0">
                <div class="text-normal">
                    <label><%-- <egov-smc:fieldLabel name="Application No."
							compulsory="true" />  --%>Application No.</label><span class="text"><%=appNo%></span> <small
                        class="error"></small>
                </div>

                <div class="text-normal">
                    <label><%-- <egov-smc:fieldLabel name="Current Status"
							compulsory="true" /> --%>Current Status</label> <span class="text"><%=displayCurrentStatus%></span>
                    <small class="error"></small>
                </div>

                <div class="text-normal">
                    <label><%-- <egov-smc:fieldLabel name="Update Status"
							compulsory="true" /> --%>Update Status</label> <span class="text" id="updateStatusDis"></span>
                    <small class="error"></small>
                </div>

                <div class="text-normal" id="updateStatusComments">
                    <label><%-- <egov-smc:fieldLabel name="Comments"
							compulsory="true" /> --%>Comments</label>
            ...
            <div id="msgTab2-1" style="<c:out value='${cssEmail }'/>">
                <form id="emailInputForm2">
                    <div class="text-normal">
                        <label><%-- <egov-smc:fieldLabel name="To" compulsory="true" /> --%>To</label>
                    ...
                    <div class="text-normal">
                        <label><%-- <egov-smc:fieldLabel name="Title" compulsory="true" /> --%>Title</label>
                    ...
                    <div class="text-normal">
                        <label><%-- <egov-smc:fieldLabel name="Content"
								compulsory="true" /> --%>Content</label>
            ...
            <div id="msgTab2-2" style="<c:out value='${cssSMS }'/>">
                <form id="smsInputForm2">
                    ...
                    <div class="text-normal">
                        <label><%-- <egov-smc:fieldLabel name="Content"
								compulsory="true" /> --%>Content</label> <span class="text"><textarea
                            rows="4" class="width-350px" cols="45" id="smsContent2"
                            name="smsContent2"></textarea> <br> <span
                            style="margin-top: 5px; color: #666;">Characters left : <span
    ...
    <script type="text/javascript">
        ...
        function validateUploadInternal(){
            ...
            if(name==""){
                ...
                $('[name="internalDocName"]').nextAll('small.error').html("The length must less than ["+nameLength+"]");
                flag = flag && false;
            }else if(internalDoc[name.toLowerCase()]){
                $('[name="internalDocName"]').nextAll('small.error').html("Name("+name+") is duplicated");
                flag = flag && false;
            }
            ...
            if(internalFile==""){
                ...
                $('[name="internalFile"]').nextAll('small.error').html("The file name must less than ["+fileNameLength+"]");
                flag = flag && false;
            }
        ...
        function validateUploadSupportDoc(){
            ...
            if(name==""){
                ...
                $('[name="documentName"]').nextAll('small.error').html("The length must less than ["+nameLength+"]");
                flag = flag && false;
            }
            ...
            if(supportFile==""){
                ...
                $('[name="supportFile"]').nextAll('small.error').html("The file name must less than ["+fileNameLength+"]");
                flag = flag && false;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data,function(key,value){
                        ...
                        trStr +="<td>"+userId+"</td>";
                        trStr +="<td>"+formatDate(value.createdDate)+"</td></tr>";
                    })
