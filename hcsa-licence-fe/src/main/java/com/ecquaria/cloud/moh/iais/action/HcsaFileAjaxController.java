...
public class HcsaFileAjaxController {
    ...
    @PostMapping(value = "ajax-batchUpload-file",produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public String batchUpload(HttpServletRequest request,@RequestParam("selectedFile") MultipartFile selectedFile){
        MessageDto messageCode = new MessageDto();
        FileRepoDto fileRepoDto = new FileRepoDto();
        fileRepoDto.setFileName(selectedFile.getOriginalFilename());
        fileRepoDto.setAuditTrailDto(IaisEGPHelper.getCurrentAuditTrailDto());
        fileRepoDto.setRelativePath(AppConsts.FALSE);
        String auditTrailStr = com.ecquaria.cloud.moh.iais.common.utils.JsonUtil.parseToJson(fileRepoDto);
        String res = fileRepoClient.saveFiles(selectedFile,auditTrailStr).getEntity();

        InputFilesDto inputFilesDto = new InputFilesDto();
        inputFilesDto.setFileName(selectedFile.getOriginalFilename());
        inputFilesDto.setMainFileName(selectedFile.getOriginalFilename());
        inputFilesDto.setFileType("INFI001");
        inputFilesDto.setRecordsNum(0);
        inputFilesDto.setProcessedRecordsNum(0);
        inputFilesDto.setErrorRecordsNum(0);
