...
//        ParamUtil.setRequestAttr(bpc.request,"counselling",COUNSELLING);
    ...
    private void retrieveHciCode(HttpServletRequest request ,TopSuperDataSubmissionDto topSuperDataSubmissionDto) {
        ...
        if (premisesMap.size() !=0) {
            ...
            premisesDtos.sort(Comparator.comparing(PremisesDto::getHciName, Comparator.nullsFirst(Comparator.naturalOrder())).reversed());
    ...
    public void doSubmission(BaseProcessClass bpc) throws ParseException {
        ...
        if(!isRfc(bpc.request)){
            emailAddress = DataSubmissionHelper.getEmailAddrsByRoleIdsAndLicenseeId(bpc.request, Collections.singletonList(RoleConsts.USER_ROLE_DS_TOP));
        } else {
            emailAddress = DataSubmissionHelper.getEmailAddrsByRoleIdsAndLicenseeId(bpc.request, Collections.singletonList(RoleConsts.USER_ROLE_DS_TOP_SUPERVISOR));
        }
