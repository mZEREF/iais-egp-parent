package com.ecquaria.cloud.moh.iais.api.config;

import com.ecquaria.cloud.helper.ConfigHelper;
import lombok.extern.slf4j.Slf4j;

@Slf4j
public class GatewayStripeConfig {
	

	
	public static final String payment_registry_name = ConfigHelper.getString("stripe.payment.registry.name");
	
	//MD5 key
	public static final String key = ConfigHelper.getString("stripe.payment.md5.key");//"4783D627CE96817C5D5DBA6DADECD2F15ECA5D1F97A0FDD8C07CA43B50C143B6";
	
	//RSA private key:""
	public static final String rsa_private_key = ConfigHelper.getString("rsa.private.key");
	//RSA public key:""
	public static final String rsa_public_key = ConfigHelper.getString("rsa.public.key");
	//RSA Gateway public key:""
	public static final String rsa_gateway_public_key = ConfigHelper.getString("rsa.gateway.private.key");
	
	public static final String seller_email = ConfigHelper.getString("stripe.payment.seller.email");

	//public static String notify_url = "http://192.168.6.80:8088/egp/commpymt/commpymt_notify.jsp";
	public static final String notify_url = ConfigHelper.getString("stripe.payment.notify.url");
	
	//"http://192.168.6.80:8088/egp/commpymt/commpymt_return.jsp";
	public static final String return_url = ConfigHelper.getString("stripe.payment.return.url");

	//"http://192.168.6.80:8088/egp/process/EGOV/CommPayment?";
	public static final String common_gateway_url = ConfigHelper.getString("stripe.payment.gateway.url");
	//public static final String common_gateway_url = "/payment-web/process/EGPCLOUD/CommPayment?";
	
	//"http://192.168.6.80:8088/egp/process/EGOV/PaymentGatewayService?";
	public static final String common_gateway_service_url = ConfigHelper.getString("stripe.payment.gateway.service.url");

	public static final String retrigger_giro_paymeny_return_rul = ConfigHelper.getString("stripe.payment.retrigger.giro.pmt.return.url");
	
	//"D:\\alipay_log_" + System.currentTimeMillis() + ".txt";
	public static final String log_path = ConfigHelper.getString("stripe.payment.log.path");

	public static final String input_charset = ConfigHelper.getString("stripe.payment.input.charset");//UTF_8

	public static final String sign_type = ConfigHelper.getString("stripe.payment.sign.type");//RSA
	
	public static final String dtLong                  = "yyyyMMddHHmmss";
    
    public static final String simple                  = "yyyy-MM-dd HH:mm:ss";
    
    public static final String dtShort                 = "yyyyMMdd";
	
}
