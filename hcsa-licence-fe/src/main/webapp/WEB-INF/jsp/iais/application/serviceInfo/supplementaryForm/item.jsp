    ...
    <c:choose>
                ...
                <c:forEach var="idx" begin="0" end="${item.codes.size() - 1}">
                    <div class="col-xs-12 col-md-${item.labelWidth} form-check">
                        <input class="form-check-input" type="radio" ${itemData}
                               name="${itemConfigDto.id}${item.seqNum}" value="${item.codes[idx]}"
                               data-id="${itemConfigDto.id}${item.codes[idx]}" id="${itemConfigDto.id}${item.codes[idx]}${item.seqNum}"
        ...
        <c:when test="${isCheckBox}">
            <input class="form-check-input" type="checkbox" ${itemData}
                   name="${itemConfigDto.radioBatchNum}${item.seqNum}" value="${itemConfigDto.id}"
                   data-id="${itemConfigDto.id}" id="${itemConfigDto.id}${item.seqNum}"
            ...
            <iais:value width="7" cssClass="col-md-7">
                <c:if test="${item.date}">
                    <iais:input type="text" data="${itemData} ${item.endHtml} ${item.startHtml}" cssClass="date_picker"
                                name="${itemConfigDto.id}${item.seqNum}" value="${item.inputValue}" placeholder="dd/mm/yyyy" />
                </c:if>
                <c:if test="${not item.date}">
                    <iais:input type="text" data="${itemData}" maxLength="${itemConfigDto.maxLength}"
                                name="${itemConfigDto.id}${item.seqNum}" value="${item.inputValue}" />
                </c:if>
                <span class="error-msg " name="iaisErrorMsg" id="error_${itemConfigDto.id}${item.seqNum}"></span>
            </iais:value>
        </c:when>
