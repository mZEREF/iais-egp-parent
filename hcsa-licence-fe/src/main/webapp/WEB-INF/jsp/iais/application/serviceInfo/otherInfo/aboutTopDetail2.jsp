...
<c:if test="${('1' == appSvcOtherInfoTop.topType) || ('0' == provideTop) || (empty appSvcOtherInfoTop.topType)}">hidden</c:if>" data-prefix="${prefix}">
    <iais:row>
        <div class="col-xs-12 col-md-10">
            <p class="bold">TOP (By Surgical Procedure)&nbsp;<label class="assign-psn-item" data-prefix="${prefix}"><c:if test="${topBySurgicalProcedure.size() > 1}">${index+1}</c:if></label></p>
        </div>
    ...
    <iais:row>
        ...
        <iais:value width="7" cssClass="col-md-7">
            <iais:input maxLength="4" type="number" cssClass="pyear" name="${prefix}pyear${index}" value="${person.year}"/>
        </iais:value>
    </iais:row>
    ...
    <iais:row>
        ...
        <iais:value width="7" cssClass="col-md-7">
            <iais:input maxLength="7" type="number" cssClass="pabortNum" name="${prefix}pabortNum${index}" value="${person.abortNum}"/>
        </iais:value>
    </iais:row>
</div>
