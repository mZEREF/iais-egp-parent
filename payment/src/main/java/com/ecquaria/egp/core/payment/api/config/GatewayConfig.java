package com.ecquaria.egp.core.payment.api.config;

import com.ecquaria.cloud.helper.ConfigHelper;
import lombok.extern.slf4j.Slf4j;

@Slf4j
public class GatewayConfig {
	

	
	public static final String payment_registry_name = ConfigHelper.getString("payment.registry.name");
	
	//MD5 key
	public static final String key = ConfigHelper.getString("payment.md5.key");//"4783D627CE96817C5D5DBA6DADECD2F15ECA5D1F97A0FDD8C07CA43B50C143B6";
	
	//RSA private key:""
	public static final String rsa_private_key = ConfigHelper.getString("rsa.private.key");
	//RSA public key:""
	public static final String rsa_public_key = ConfigHelper.getString("rsa.public.key");
	//RSA Gateway public key:""
	public static final String rsa_gateway_public_key = ConfigHelper.getString("rsa.gateway.private.key");
	
	public static final String seller_email = ConfigHelper.getString("payment.seller.email");

	//public static String notify_url = "http://192.168.6.80:8088/egp/commpymt/commpymt_notify.jsp";
	public static final String notify_url = ConfigHelper.getString("payment.notify.url");
	
	//"http://192.168.6.80:8088/egp/commpymt/commpymt_return.jsp";
	public static final String return_url = ConfigHelper.getString("payment.return.url");

	public static final String stripeKey = ConfigHelper.getString("stripe.api.skey");

	public static final String stripePKey = ConfigHelper.getString("stripe.api.pkey");

	public static final String eNetsUmId = ConfigHelper.getString("eNets.api.umid");

	public static final String eNetsJqueryUrl = ConfigHelper.getString("eNets.jquery.js.url");

	public static final String eNetsAppsUrl = ConfigHelper.getString("eNets.apps.js.url");

	public static final String eNetsEnvUrl = ConfigHelper.getString("eNets.env.jsp.url");

	public static final String eNetsKeyId = ConfigHelper.getString("eNets.key.id");

	public static final String eNetsSecretKey = ConfigHelper.getString("eNets.secret.key");

	public static final String eNetsPaymentMethod = ConfigHelper.getString("eNets.payment.method");

	//"http://192.168.6.80:8088/egp/process/EGOV/CommPayment?";
	public static final String common_gateway_url = ConfigHelper.getString("payment.gateway.url");
	//public static final String common_gateway_url = "/payment-web/process/EGPCLOUD/CommPayment?";
	
	//"http://192.168.6.80:8088/egp/process/EGOV/PaymentGatewayService?";
	public static final String common_gateway_service_url = ConfigHelper.getString("payment.gateway.service.url");

	public static final String refreshTime = ConfigHelper.getString("stripe.payment.refresh.seconds");// refresh

	//"D:\\alipay_log_" + System.currentTimeMillis() + ".txt";
	public static final String log_path = ConfigHelper.getString("payment.log.path");

	public static final String input_charset = ConfigHelper.getString("payment.input.charset");//UTF_8

	public static final String sign_type = ConfigHelper.getString("payment.sign.type");//RSA
	
	public static final String dtLong                  = "yyyyMMddHHmmss";
    
    public static final String simple                  = "yyyy-MM-dd HH:mm:ss";
    
    public static final String dtShort                 = "yyyyMMdd";
	
}
