...
public class ExportNotificationDto {
    ...
    public void reqObjectMapping(HttpServletRequest request){
        ...
        for (int i = 0; i < amt; i++) {
            ...
            exportList.setReceivedFacility(ParamUtil.getString(request,KEY_PREFIX_RECEIVED_FACILITY+SEPARATOR+i));
            exportList.setReceivedCountry(ParamUtil.getString(request,KEY_PREFIX_RECEIVED_COUNTRY+SEPARATOR+i));
            exportList.setExportDate(ParamUtil.getString(request,KEY_PREFIX_EXPORT_DATE +SEPARATOR+i));
            exportList.setProvider(ParamUtil.getString(request,KEY_PREFIX_PROVIDER+SEPARATOR+i));
            exportList.setFlightNo(ParamUtil.getString(request,KEY_PREFIX_FLIGHT_NO+SEPARATOR+i));
            exportList.setRemarks(ParamUtil.getString(request,KEY_PREFIX_REMARKS+SEPARATOR+i));
