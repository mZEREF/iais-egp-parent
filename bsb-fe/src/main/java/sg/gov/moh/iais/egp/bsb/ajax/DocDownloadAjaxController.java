...
public class DocDownloadAjaxController {
    ...
    @GetMapping("/facReg/committee/sample/{id}")
    public void downloadFacCommitteeSampleFile(@PathVariable("id") String maskedFileRepoId, HttpServletRequest request, HttpServletResponse response) {
        downloadFile(request, response, maskedFileRepoId, this::unmaskFileId, this::facRegCommitteeSampleFile);
    }

    @GetMapping("/facReg/authoriser/sample/{id}")
    public void downloadFacAuthoriserSampleFile(@PathVariable("id") String maskedFileRepoId, HttpServletRequest request, HttpServletResponse response) {
        downloadFile(request, response, maskedFileRepoId, this::unmaskFileId, this::facRegAuthoriserSampleFile);
    }
    ...
    @GetMapping("/adhocRfi/new/{id}")
    public void downloadNewAdhocRfiFile(@PathVariable("id") String maskedRepoId, HttpServletRequest request, HttpServletResponse response) {
        downloadFile(request, response, maskedRepoId, this::unmaskFileId, this::adhocRfiGetNewFile);
    }
    ...
    public void downloadSavedWithdrawnFile(@PathVariable("id") String maskedRepoId, HttpServletRequest request, HttpServletResponse response) {
    ...
    @GetMapping("/final/inspection/{id}")
    public void downloadInspectionFile(@PathVariable("id") String maskedTmpId, HttpServletRequest request, HttpServletResponse response) {
        downloadFile(request, response, maskedTmpId, this::unmaskFileId, this::getFinalInspectionReport);
    }

    @GetMapping("/final/certification/{id}")
    public void downloadCertificationFile(@PathVariable("id") String maskedTmpId, HttpServletRequest request, HttpServletResponse response) {
        downloadFile(request, response, maskedTmpId, this::unmaskFileId, this::getFinalCertificationReport);
    }
    @GetMapping("/final/inspectionAndCertification/{id}")
    public void downloadInsAndCerFile(@PathVariable("id") String maskedTmpId, HttpServletRequest request, HttpServletResponse response) {
        downloadFile(request, response, maskedTmpId, this::unmaskFileId, this::getFinalCertificationInspectionReport);
    }
    ...
    /** Facility registration download biosafety committee sample file  */
    private MultipartFile facRegCommitteeSampleFile(HttpServletRequest request, String id) {
        SampleFileDto committeeSampleFileDto = (SampleFileDto) ParamUtil.getSessionAttr(request, FacRegisterConstants.KEY_SAMPLE_COMMITTEE);
        if (!committeeSampleFileDto.getFileRepoId().equals(id)) {
            throw new IllegalStateException(ERROR_MESSAGE_INVALID_ID);
        }
        byte[] content = fileRepoClient.getFileFormDataBase(id).getEntity();
        return new ByteArrayMultipartFile(null, committeeSampleFileDto.getName(), null, content);
    }

    /** Facility registration download authoriser sample file  */
    private MultipartFile facRegAuthoriserSampleFile(HttpServletRequest request, String id) {
        SampleFileDto authoriserSampleFileDto = (SampleFileDto) ParamUtil.getSessionAttr(request, FacRegisterConstants.KEY_SAMPLE_AUTHORISER);
        if (!authoriserSampleFileDto.getFileRepoId().equals(id)) {
            throw new IllegalStateException(ERROR_MESSAGE_INVALID_ID);
        }
        byte[] content = fileRepoClient.getFileFormDataBase(id).getEntity();
        return new ByteArrayMultipartFile(null, authoriserSampleFileDto.getName(), null, content);
    }
    ...
    private MultipartFile withdrawnGetSavedFile(HttpServletRequest request, String id) {
        sg.gov.moh.iais.egp.bsb.dto.file.PrimaryDocDto docDto = (sg.gov.moh.iais.egp.bsb.dto.file.PrimaryDocDto) ParamUtil.getSessionAttr(request, PARAM_PRIMARY_DOC_DTO);
        DocRecordInfo info = docDto.getSavedDocMap().get(id);
        if (info == null) {
            throw new IllegalStateException(ERROR_MESSAGE_RECORD_INFO_NULL);
        }
        byte[] content = fileRepoClient.getFileFormDataBase(id).getEntity();
    ...
    private MultipartFile adhocRfiGetNewFile(HttpServletRequest request, String id) {
        sg.gov.moh.iais.egp.bsb.dto.file.PrimaryDocDto docDto = (sg.gov.moh.iais.egp.bsb.dto.file.PrimaryDocDto) ParamUtil.getSessionAttr(request, PARAM_PRIMARY_DOC_DTO);
        return docDto.getNewDocMap().get(id).getMultipartFile();
    }

    private MultipartFile getFinalInspectionReport(HttpServletRequest request, String insCerFacRelId){
        List<Map<String, byte[]>> fileNameFileDataMap = getFileNameFileDataMap(insCerFacRelId,MasterCodeConstants.DATA_TYPE_INS);
        if (CollectionUtils.isEmpty(fileNameFileDataMap)) {
            throw new IllegalStateException(ERROR_MESSAGE_RECORD_INFO_NULL);
        }
        Map<String, byte[]> map = fileNameFileDataMap.get(0);
        // the fileNameFileDataMap only have one value
        String fileName = map.keySet().iterator().next();
        return new ByteArrayMultipartFile(null, fileName, null, map.get(fileName));
    }

    // Convert multiple Certification files into a single ZIP file for users to download
    private MultipartFile getFinalCertificationReport(HttpServletRequest request, String insCerFacRelId){
        List<Map<String, byte[]>> fileNameFileDataMap = getFileNameFileDataMap(insCerFacRelId,MasterCodeConstants.DATA_TYPE_CER);
        if (CollectionUtils.isEmpty(fileNameFileDataMap)) {
            throw new IllegalStateException(ERROR_MESSAGE_RECORD_INFO_NULL);
        }
        byte[] fileData = getFileData(fileNameFileDataMap);
        return new ByteArrayMultipartFile(null, "Certification.zip", null, fileData);
    }

    // Convert multiple Certification files and Inspection file into a single ZIP file for users to download
    private MultipartFile getFinalCertificationInspectionReport(HttpServletRequest request, String insCerFacRelId){
        List<Map<String, byte[]>> fileNameFileDataMap = getFileNameFileDataMap(insCerFacRelId,MasterCodeConstants.DATA_TYPE_ALL);
        if (CollectionUtils.isEmpty(fileNameFileDataMap)) {
            throw new IllegalStateException(ERROR_MESSAGE_RECORD_INFO_NULL);
        }
        byte[] fileData = getFileData(fileNameFileDataMap);
        return new ByteArrayMultipartFile(null, "Inspection&Certification.zip", null, fileData);
    }

    /**
     * get data for download
     * @see #getFileData(List)
     * @param insCerFacRelId InspectionCertificationFacilityRel id
     * @param type possible value: 'INS','CER','ALL'
     * @return List<Map<fileName,fileData>>
     */
    public List<Map<String,byte[]>> getFileNameFileDataMap(String insCerFacRelId, String type) {
        ...
        Boolean haveCertification = stringBooleanMap.get(MasterCodeConstants.DATA_TYPE_CER);
        Boolean haveInspection = stringBooleanMap.get(MasterCodeConstants.DATA_TYPE_INS);

        List<Map<String,byte[]>> list = new ArrayList<>();
        if (haveCertification == Boolean.TRUE && (MasterCodeConstants.DATA_TYPE_CER.equals(type) || MasterCodeConstants.DATA_TYPE_ALL.equals(type))) {
            List<Map<String, byte[]>> cerRepoIdFileNameMapList = inspectionAFCClient.getCerFileNameFileDataMapByInsCerFacRelId(insCerFacRelId);
            list.addAll(cerRepoIdFileNameMapList);
        }
        if (haveInspection == Boolean.TRUE && (MasterCodeConstants.DATA_TYPE_INS.equals(type) || MasterCodeConstants.DATA_TYPE_ALL.equals(type))) {
            Map<String, byte[]> map = inspectionClient.getInsFileNameFileDataMapByInsCerFacRelId(insCerFacRelId);
            list.add(map);
        }
        return list;
    }
    ...
    /**
     *
     * @param list List<Map<fileName,fileData>>
     * @return zip data for user to download
     */
    public byte[] getFileData(List<Map<String,byte[]>> list) {
        byte[] data = null;
        if (!CollectionUtils.isEmpty(list)) {
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            ZipOutputStream zipOut = new ZipOutputStream(baos);
            ByteArrayInputStream bais = null;
            // compress files into one zip file, here we use system time to avoid duplicate filename
            try {
                for (Map<String, byte[]> stringMap : list) {
                    String fileName = stringMap.keySet().iterator().next();
                    bais = new ByteArrayInputStream(stringMap.get(fileName));
                ...
                log.error("Fail to retrieve file data", e);
            }
        }
        return data;
    }
