...
import static sg.gov.moh.iais.egp.bsb.constant.module.ApprovalBatAndActivityConstants.KEY_EDIT_JUDGE;
...
import static sg.gov.moh.iais.egp.bsb.constant.module.ApprovalBatAndActivityConstants.KEY_RFC_APPROVAL_ID;
...
@Slf4j
@Delegator("rfcApprovalBatAndActivityDelegator")
@RequiredArgsConstructor
public class RfcApprovalBatAndActivityDelegator {
    ...
    public void start(BaseProcessClass bpc) {
        ...
        session.removeAttribute(SubmissionBlockConstants.BLOCK_RFC_APPROVAL);
        ...
        session.removeAttribute(KEY_OLD_APPROVAL_BAT_AND_ACTIVITY_DTO);
    ...
    public void init(BaseProcessClass bpc) {
        HttpServletRequest request = bpc.request;
        ApprovalBatAndActivityDto editDto = new ApprovalBatAndActivityDto();
        // check if we are doing editing
        String maskedApprovalId = request.getParameter(KEY_RFC_APPROVAL_ID);
        if (StringUtils.hasLength(maskedApprovalId)) {
            if (log.isInfoEnabled()) {
                log.info("masked approval ID: {}", LogUtil.escapeCrlf(maskedApprovalId));
            }
            String approvalId = MaskUtil.unMaskValue(KEY_RFC_APPROVAL_ID, maskedApprovalId);
            if (approvalId != null && !maskedApprovalId.equals(approvalId)) {
                editDto = approvalBatAndActivityService.getRfcBatDataByApprovalId(approvalId);
            }
        }
        ...
        if (StringUtils.hasLength(maskedAppId)) {
            ...
            if (appId != null && !maskedAppId.equals(appId)) {
                editDto = approvalBatAndActivityService.getEditDtoData(appId);
            }
        }
        NodeGroup approvalAppRoot = editDto.toApprovalAppRootGroup(KEY_ROOT_NODE_GROUP, ValidationConstants.VALIDATION_PROFILE_RFC);
        ...
        ParamUtil.setSessionAttr(request, KEY_OLD_APPROVAL_BAT_AND_ACTIVITY_DTO, editDto);
    ...
    public void preRfcView(BaseProcessClass bpc) {
        ParamUtil.setRequestAttr(bpc.request, KEY_EDIT_JUDGE, Boolean.TRUE);
        approvalBatAndActivityService.prepareRfcPreviewData(bpc.request);
    }

    public void handleRfcView(BaseProcessClass bpc) {
        approvalBatAndActivityService.handleRfcViewData(bpc);
    }
    ...
    public void handleFacProfile(BaseProcessClass bpc) {
        approvalBatAndActivityService.handleFacProfile(bpc, MasterCodeConstants.APP_TYPE_RFC);
    }

    public void prePossessBatDetails(BaseProcessClass bpc) {
        ParamUtil.setRequestAttr(bpc.request, KEY_EDIT_JUDGE, Boolean.TRUE);
        approvalBatAndActivityService.prePossessBatDetails(bpc, RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET);
    }

    public void handlePossessBatDetails(BaseProcessClass bpc) {
        approvalBatAndActivityService.handlePossessBatDetails(bpc, new FieldEditableSetJudger(RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET), MasterCodeConstants.APP_TYPE_RFC);
    }

    public void preLargeBatDetails(BaseProcessClass bpc) {
        ParamUtil.setRequestAttr(bpc.request, KEY_EDIT_JUDGE, Boolean.TRUE);
        approvalBatAndActivityService.preLargeBatDetails(bpc, RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET);
    }

    public void handleLargeBatDetails(BaseProcessClass bpc) {
        approvalBatAndActivityService.handleLargeBatDetails(bpc, new FieldEditableSetJudger(RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET), MasterCodeConstants.APP_TYPE_RFC);
    }

    public void preSpecialBatDetails(BaseProcessClass bpc) {
        ParamUtil.setRequestAttr(bpc.request, KEY_EDIT_JUDGE, Boolean.TRUE);
        approvalBatAndActivityService.preSpecialBatDetails(bpc, RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET);
    }

    public void handleSpecialBatDetails(BaseProcessClass bpc) {
        approvalBatAndActivityService.handleSpecialBatDetails(bpc, new FieldEditableSetJudger(RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET), MasterCodeConstants.APP_TYPE_RFC);
    }

    public void preFacAuthorised(BaseProcessClass bpc) {
        ParamUtil.setRequestAttr(bpc.request, KEY_EDIT_JUDGE, Boolean.TRUE);
        approvalBatAndActivityService.preFacAuthorised(bpc, RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET);
    }

    public void handleFacAuthorised(BaseProcessClass bpc) {
        approvalBatAndActivityService.handleFacAuthorised(bpc, new FieldEditableSetJudger(RfcFlowHardCodeDecider.APPROVAL_RFC_EDIT_FIELD_SET),MasterCodeConstants.APP_TYPE_RFC);
    }
    ...
    public void handlePrimaryDoc(BaseProcessClass bpc) {
        approvalBatAndActivityService.handlePrimaryDoc(bpc, MasterCodeConstants.APP_TYPE_RFC);
    }
        ...
        if (KEY_ACTION_JUMP.equals(actionType)) {
            if (KEY_NAV_NEXT.equals(actionValue)) {
                if (previewNode.doValidation()) {
                    ...
                    if (rfcFlowType == null) {
                        rfcFlowType = RfcFlowType.DO_NOTHING;
                    }
                    Boolean needTask;
                    ...
                    submissionBlockService.blockSubmission(request, SubmissionBlockConstants.BLOCK_RFC_APPROVAL);

                    ResponseDto<AppMainInfo> responseDto = approvalBatAndActivityClient.saveRfcApplicationToApproval(approvalBatAndActivityDto, needTask);
                    log.info("save rfc application to approval response: {}", org.apache.commons.lang.StringUtils.normalizeSpace(responseDto.toString()));
    ...
    public void actionFilter(BaseProcessClass bpc) {
        approvalBatAndActivityService.actionFilter(bpc, MasterCodeConstants.APP_TYPE_RFC);
    }
