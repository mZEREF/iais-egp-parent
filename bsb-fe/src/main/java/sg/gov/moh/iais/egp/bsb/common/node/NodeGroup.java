...
public class NodeGroup extends Node {
    ...
    public void invalidationWave(String nodeName) {
        ...
        while (nodeIter.hasNext()) {
            ...
            if (member.isValidated() && member.dependsOn(nodeName)) {
                if (member instanceof NodeGroup){
                    ((NodeGroup) member).needValidationRecursion();
                } else {
    ...
    /**
     * This method is recursion call {@link #needValidation()} for nodes.
     */
    public void needValidationRecursion() {
        needValidation();
        for (Node child : this.nodes.values()){
            if (child instanceof NodeGroup) {
                ((NodeGroup) child).needValidationRecursion();
            } else {
                child.needValidation();
            }
        }
    }
}
