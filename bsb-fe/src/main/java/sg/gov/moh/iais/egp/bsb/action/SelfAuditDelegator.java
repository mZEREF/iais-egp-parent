...
public class SelfAuditDelegator {
    ...
    public static void setAuditDoc(HttpServletRequest request, FacilitySubmitSelfAuditDto auditDto) {
        if (!CollectionUtils.isEmpty(auditDto.getDocRecordInfos())) {
            PrimaryDocDto primaryDocDto = new PrimaryDocDto();
            primaryDocDto.setSavedDocMap(sg.gov.moh.iais.egp.bsb.util.CollectionUtils.uniqueIndexMap(PrimaryDocDto.DocRecordInfo::getRepoId, auditDto.getDocRecordInfos()));
