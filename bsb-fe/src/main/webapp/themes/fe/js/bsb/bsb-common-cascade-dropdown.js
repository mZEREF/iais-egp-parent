...
/* preFunc is called at the start of the change event, postFunc is called at the end of the change event */
function registerCascadeEventWithDisabled(triggerDropDownId, targetDropdownId, dataMap, dataSet, preFunc, postFunc) {
    $("#"+triggerDropDownId).change(function () {
        if (preFunc) {preFunc();}
        changeOptionItemsFromDataMapAndDataSet(targetDropdownId, dataMap, dataSet, this.value);
        if (postFunc) {postFunc();}
    });
}
...
/* The value comes from the trigger dropdown.
 * If the value can not be found in the data map, nothing will happen */
function changeOptionItemsFromDataMapAndDataSet(dropDownId, dataMap, dataSet, value) {
    if (dataMap[value]) {
        changeOptionItemsWithDisabled(dropDownId, dataMap[value], dataSet);
    }
}


function changeOptionItemsWithDisabled(dropDownId, optionItemList, greyOptionItemSet) {
    var dropdown = document.getElementById(dropDownId);
    // delete all options
    dropdown.length = 0;
    ...
    for (const batInfo of optionItemList) {
        option = new Option(batInfo.text, batInfo.value, false, false);
        //if this bat info is exist in facility ,set option attribute disabled

        for (const grey of greyOptionItemSet){
            if(batInfo.value === grey){
