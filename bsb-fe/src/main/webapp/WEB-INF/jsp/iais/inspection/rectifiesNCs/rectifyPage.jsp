...
<%
    sg.gov.moh.iais.egp.bsb.config.BsbParamConfig paramConfig = com.ecquaria.cloud.helper.SpringContextHelper.getContext().getBean(sg.gov.moh.iais.egp.bsb.config.BsbParamConfig.class);
    String fileMaxSize = String.valueOf(paramConfig.getMaxFileSize());
%>
                    ...
                    <div class="form-horizontal">
                        <div class="col-md-10">
                            <div class="row" style="font-weight: 700;text-align: center">
                                <div class="col-md-2">Checklist Item</div>
                            ...
                            <div class="row" style="text-align: center;border-top:1px solid #D1D1D1;padding: 10px 0 ">
                                <div class="col-md-2"><c:out value="${rectifyItemDto.checklistItem}"/></div>
                                <div class="col-md-3"><c:out value="${rectifyItemDto.itemDesc}"/></div>
                                ...
                                <div class="col-md-2">
                                    <c:choose>
                                        <c:when test="${rectifyItemDto.remark ne null && rectifyItemDto.remark ne ''}"><c:out value="${rectifyItemDto.remark}"/></c:when>
                            ...
                            <div class="row" style="margin: 50px 0">
                                <ul>
                                    <li>The maximum file size per document is <%=fileMaxSize%> MB.</li>
                                        ...
                                        <c:if test="${newSavedDoc ne null}">
                                            <c:forEach var="info" items="${newSavedDoc}">
                                                ...
                                                <div id="${tmpId}FileDiv">
                                                    <a href="/bsb-web/ajax/doc/download/insNonCompliance/new/${tmpId}" style="text-decoration: underline"><span id="${tmpId}Span">${info.filename}</span></a>(<fmt:formatNumber value="${info.size/1024.0}" type="number" pattern="0.0"/>KB)<label class="other-doc-type-label">
                                        ...
                                        <c:if test="${oldSavedDoc ne null}">
                                            <c:forEach var="info" items="${oldSavedDoc}">
                                                ...
                                                <div id="${repoId}FileDiv">
                                                    <a href="/bsb-web/ajax/doc/download/insNonCompliance/repo/${repoId}" style="text-decoration: underline"><span id="${repoId}Span">${info.filename}</span></a>(<fmt:formatNumber value="${info.size/1024.0}" type="number" pattern="0.0"/>KB)<label class="other-doc-type-label">
                                                    (<iais:code code="${info.docType}"/>)</label><button
