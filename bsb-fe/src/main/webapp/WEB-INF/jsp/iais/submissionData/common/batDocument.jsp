<input type="hidden" id="existFiles--v--${status.index}" name="existFiles--v--${status.index}" value="${reqT.repoIdNewString}">
        ...
        <div id="PrimaryDocsPanel" role="tabpanel">
            <div class="document-content" style="margin: 0 auto;background-color: #F2F2F2;height: 150px">
                <div class="document-upload-list--v--${status.index}">
                    ...
                    <c:set var="savedFiles" value="${reqT.savedInfos}"/>
                    <c:set var="newFiles" value="${reqT.newDocInfos}"/>
                    <h3>${doc.get(reqT.docType)}<c:if test="${doc.mandatory}"> <span class="mandatory otherQualificationSpan">*</span></c:if></h3>
