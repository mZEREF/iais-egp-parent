...
<%@attribute name="savedFiles" required="true" type="java.util.List<sg.gov.moh.iais.egp.bsb.dto.file.DocRecordInfo>" %>
                                ...
                                <c:if test="${savedFiles ne null}">
                                    <c:forEach var="info" items="${savedFiles}">
                                        ...
                                        <div id="${tmpId}FileDiv">
                                            <a href="${savedFileDLUrl}${tmpId}/${info.filename}" style="text-decoration: underline"><span id="${tmpId}Span"><c:out value="${info.filename}"/></span></a>(<fmt:formatNumber value="${info.size/1024.0}" type="number" pattern="0.0"/>KB)
