...
<%@attribute name="scheduleOps" required="true" type="java.util.List<sg.gov.moh.iais.egp.common.modal.view.RichSelectOption>" %>
...
<c:set var="onlyFifthSchedule" value="${masterCodeConstants.FIFTH_SCHEDULE eq firstScheduleOp}"/>
<c:set var="BAorT" value='${onlyFifthSchedule ? "Toxin" : "Biological Agent"}'/>
                ...
                <c:forEach var="info" items="${batInfos}" varStatus="status">
                    <section id="batInfoSection--v--${status.index}">
                        <div class="form-group ">
                                    ...
                                    <c:otherwise>
                                        <select name="schedule--v--${status.index}" class="scheduleDropdown${status.index}" id="schedule--v--${status.index}" data-cascade-dropdown="schedule-bat">
                                            ...
                                            <c:forEach items="${scheduleOps}" var="schedule">
                                                <option value="${schedule.value}" <c:if test="${schedule.disable}">disabled="disabled"</c:if> <c:if test="${info.schedule eq schedule.value}">selected="selected"</c:if>>${schedule.text}</option>
                                            </c:forEach>
                                        </select>
                            ...
                            <c:if test="${status.index gt 0 and !editJudge}">
                                <div class="col-sm-1"><h4 class="text-danger"><em id="removeBtn--v--${status.index}" data-current-idx="${status.index}" class="fa fa-times-circle del-size-36 cursorPointer removeBtn"></em></h4></div>
                            </c:if>
                        </div>
                        <div class="form-group ">
                            <div class="col-sm-5 control-label">
                            ...
                            <div class="col-sm-6" style="z-index: 20;">
                                <select name="batName--v--${status.index}" class="batNameDropdown${status.index}" id="batName--v--${status.index}">
                                    <c:set var="batNameOps" value="${onlyFifthSchedule ? scheduleBatMap.get(firstScheduleOp) : scheduleBatMap.get(info.schedule)}"/>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <c:forEach items="${batNameOps}" var="name">
                                                <option value="${name.value}" <c:if test="${name.disable}">disabled="disabled"</c:if> <c:if test="${info.batName eq name.value}">selected="selected"</c:if>>${name.text}</option>
                                            </c:forEach>
                                        </c:otherwise>
                                    </c:choose>
                                </select>
                        ...
                        <c:if test="${not lspJudge}">
                            <div class="form-group ">
                                <div class="col-sm-5 control-label">
                                    <label>Types of sample that will be handled <span class="mandatory otherQualificationSpan">*</span></label>
                                </div>
                                <div class="col-sm-6" style="z-index: 10;">
                                    <div class="self-assessment-checkbox-gp">
                                        ...
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="sampleType--v--${status.index}" id="sampleEnv--v--${status.index}" data-custom-ind="batEvnSampleType" <c:if test="${info.sampleType.contains(masterCodeConstants.SAMPLE_NATURE_ENVIRONMENTAL)}">checked="checked"</c:if> value="${masterCodeConstants.SAMPLE_NATURE_ENVIRONMENTAL}"/>
                                            <label for="sampleEnv--v--${status.index}" class="form-check-label"><span class="check-square"></span>Environmental sample containing biological agent or toxin (specify the type of samples under details)</label>
                                        </div>
                                        <div class="form-check">
                            ...
                            <div class="form-group ">
                                <div class="col-sm-5 control-label">
                                    <label>Type of work that will be carried out involving the ${lBAorT} <span class="mandatory otherQualificationSpan">*</span></label>
                                </div>
                                <div class="col-sm-6" style="z-index: 10;">
                                    <div class="self-assessment-checkbox-gp">
                                        <div class="form-check">
                            ...
                            <div id="sampleWorkDetailDiv--v--${status.index}" class="form-group" <c:if test="${not bsb:collectionContainsAny(info.sampleType, [masterCodeConstants.SAMPLE_NATURE_ENVIRONMENTAL, masterCodeConstants.SAMPLE_NATURE_OTHER]) and not bsb:collectionContainsAny(info.workType, [masterCodeConstants.WORK_TYPE_BIOMANUFACTURING_INVOLVING_BAT, masterCodeConstants.WORK_TYPE_ANIMAL_STUDIES, masterCodeConstants.WORK_TYPE_OTHERS])}">style="display: none"</c:if>>
                                <div class="col-sm-5 control-label">
                        ...
                        <bat:batProcFacilityDetail detail="${info.details}" status="${status}" addressTypeOps="${addressTypeOps}" countryOps="${countryOps}" sourceFac="${sourceFac}" lspJudge="${lspJudge}" spFifthJudge="${spFifthJudge}"/>
                    </section>
                </c:forEach>
            </div>
            <c:if test="${!editJudge}">
