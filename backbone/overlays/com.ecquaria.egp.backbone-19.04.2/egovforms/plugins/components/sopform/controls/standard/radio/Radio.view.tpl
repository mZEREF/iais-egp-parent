...
<div id="${id}" class="form-group form-horizontal control-set-alignment formgap control ${control.properties.developerclass}">
	{if instructions}
	...
	<div class="col-sm-5">
		...
		<table{if control.properties.width > 0} width="${control.properties.width}"{/if} style="width:100%">
			...
			{for option in options}
				...
				<td {if i < cols} width="${width}" {/if}>
					<div class="control-item-container" style="margin-right:10px;padding-left:5px">
						<input type="radio" id="${id}--${option_index}"
								name="${id}" value="${option.code|escape}"
								{if text === option.code} checked {/if} />
						{if control.properties.supportEscapeOption}
							<label id="${id}--label--${option_index}"
									for="${id}--${option_index}"
									class="control-set-font control-font-normal">${option.description|escape}</label>
						{/if}
						{if !control.properties.supportEscapeOption}
							<label id="${id}--label--${option_index}"
									for="${id}--${option_index}"
									class="control-set-font control-font-normal">${option.description}</label>
						{/if}
					</div>
