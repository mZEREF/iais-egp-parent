        ...
        <table class="table">
            ...
            <c:forEach var = "item" items = "${adchklDto.adItemList}" varStatus="status">
                <tr>
                    ...
                    <td><input name="<c:out value="${item.id}"/>adhocradIns${inspectorsStatus.index}" id="<c:out value="${item.id}"/>adhocitemCheckboxNa" onclick="hideCheckBox('${ckkId}','${inspectorsStatus.index}')" type="radio" <c:if test="${inspAhocAnswer.answer eq'Na'}">checked</c:if> value="Na" ${inspectorUserFinishChecklistId == inspector.id ? "" : 'disabled'}/></td>
                    <td>
                        <textarea ${inspectorUserFinishChecklistId == inspector.id ? "" : 'disabled'} cols="70" rows="7" name="<c:out value="${item.id}"/>adhocremarkIns${inspectorsStatus.index}" id="<c:out value="${item.id}"/>adhocitemCheckboxRemarkIns${inspectorsStatus.index}" id="" maxlength="500"><c:out value="${inspAhocAnswer.remark}"/></textarea>
                    </td>
                    <td>
                        <div id="<c:out value="${item.id}"/>ckIns${inspectorsStatus.index}"<c:if test="${inspAhocAnswer.answer != 'No'}">hidden</c:if>>
                            <input name="<c:out value="${item.id}"/>adhocrecIns${inspectorsStatus.index}" id="<c:out value="${item.id}"/>adhocrec" type="checkbox" <c:if test="${inspAhocAnswer.isRec == '1'}">checked</c:if> value="rec" ${inspectorUserFinishChecklistId == inspector.id ? "" : 'disabled'}/>
                        </div>
                    </td>
                </tr>
            </c:forEach>
