...
public class AutoRenewalService {
    //system param(this is undo in config)
    ...
    public void startRenewal() {
        ...
        facDayList.add(getDay(sysParDtoFacM3));
        ...
        facCerDayList.add(getDay(sysParDtoFacCerM6));
        ...
        log.info("facility registration renewal day is {}", StringUtils.normalizeSpace(facDayList.toString()));
        log.info("facility certifier registration renewal day is {}", StringUtils.normalizeSpace(facCerDayList.toString()));
        ...
        log.info("facility registration renewal approval is {}", StringUtils.normalizeSpace(approvalFacDtoMap.toString()));
        log.info("facility registration renewal approval is {}", StringUtils.normalizeSpace(approvalFacCerDtoMap.toString()));
        List<ApprovalDto> suspendedApprovalList = autoRenewalClient.updateSuspendedApproval().getEntity();
        List<ApprovalDto> expiredApprovalList = autoRenewalClient.updateExpiredApproval().getEntity();
        log.info("suspend approval is {}", StringUtils.normalizeSpace(suspendedApprovalList.toString()));
        log.info("expired approval is {}", StringUtils.normalizeSpace(expiredApprovalList.toString()));
        if (!approvalFacDtoMap.isEmpty()){
            sendEmail(approvalFacDtoMap);
            sendEmail(approvalFacCerDtoMap);
            //if is the latest notice of the date
