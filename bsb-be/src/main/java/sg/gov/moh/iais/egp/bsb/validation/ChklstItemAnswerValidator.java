...
public class ChklstItemAnswerValidator implements CustomizeValidator {
    public static final String ERROR_MSG_FOLLOW_UP_ITEM_NOT_SELECTED = "The field must be empty when the Follow-Up Item is No or N/A.";
    ...
    public Map<String, String> validate(Object obj, String profile, HttpServletRequest request) {
        ...
        if (ChecklistConstants.ANSWER_YES.equals(answerDto.getFollowupItem())) {
            ...
            if (!StringUtil.isEmpty(answerDto.getObserveFollowup())) {
                errorMap.put("observeFollowup", ERROR_MSG_FOLLOW_UP_ITEM_NOT_SELECTED);
            }
            if (!StringUtil.isEmpty(answerDto.getFollowupAction())) {
                errorMap.put("followupAction", ERROR_MSG_FOLLOW_UP_ITEM_NOT_SELECTED);
            }
            if (!StringUtil.isEmpty(answerDto.getDueDt())) {
                errorMap.put("dueDate", ERROR_MSG_FOLLOW_UP_ITEM_NOT_SELECTED);
            }
        }
        return errorMap;
    }
