...
public interface DataSubmissionClient {
    @GetMapping(path = "/bsb-dataSubmission-be/{appId}", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseDto<MohReviewDataSubmissionDto> getOfficerReviewDataByAppId(@PathVariable("appId") String applicationId);

    @PostMapping(path = "/bsb-dataSubmission-be/form-validation/main", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    ValidationResultDto validateMohProcessDto(@RequestBody MohReviewDataSubmissionDto dto);

    @PostMapping(value = "/bsb-dataSubmission-be/do-process", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    FeignResponseEntity<Void> doProcessDataSubmission(@RequestBody MohProcessDto dto);

    @PostMapping(value = "/bsb-dataSubmission-be/ao-process", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
