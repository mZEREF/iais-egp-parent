...
public class BsbAdhocRfiDelegator {
    ...
    private static final String KEY_FACILITY_ID = "facId";
    private static final String KEY_FACILITY_STATUS = "facilityStatus";
    ...
    public void doStart(BaseProcessClass bpc){
        ...
        String maskedFacId = ParamUtil.getRequestString(request,"rowRadio");
        if (log.isInfoEnabled()) {
            log.info("masked facility id: [{}]", LogUtil.escapeCrlf(maskedFacId));
        }
        String facId = MaskUtil.unMaskValue("id", maskedFacId);
        if (facId == null || facId.equals(maskedFacId)) {
    ...
    public void preAdhocRfi(BaseProcessClass bpc){
        ...
        if (resultDto.ok()) {
            ...
            ParamUtil.setRequestAttr(request, KEY_ADHOC_RFI_LIST,  resultDto.getEntity().getRfiList());
            ParamUtil.setRequestAttr(request, KEY_FACILITY_STATUS,  resultDto.getEntity().getFacilityStatus());
        } else {
    ...
    public void preViewAdhocRfi(BaseProcessClass bpc){
        ...
        if(!StringUtils.isEmpty(viewAdhocRfiDto.getDueDate())){
            viewAdhocRfiDto.setDueDateShow(viewAdhocRfiDto.getDueDate().format(DateTimeFormatter.ofPattern(AppConstants.DEFAULT_DATE_FORMAT)));
        }
        ...
        Map<String, String> repoIdDocNameMap = DocDisplayDtoUtil.getRepoIdDocNameMap(viewAdhocRfiDto.getApplicationDocDtos());
    ...
    public void doUpdate(BaseProcessClass bpc) {
        ...
        if(!StringUtils.isEmpty(date)&&CommonValidator.isDate(date)){
            viewAdhocRfiDto.setDueDate(LocalDate.parse(date, DateTimeFormatter.ofPattern(AppConstants.DEFAULT_DATE_FORMAT)));
    ...
    public void preNewAdhocRfi(BaseProcessClass bpc){
        HttpServletRequest request = bpc.request;
        String facilityId = (String) ParamUtil.getSessionAttr(request, KEY_FACILITY_ID);
        NewAdhocRfiDto newAdhocRfiDto = adhocRfiClient.queryPreNewData(facilityId).getEntity();
        String isValidate = (String) ParamUtil.getRequestAttr(request,IS_VALIDATE);
        if(isValidate !=null && !isValidate.equals("true")){
    ...
    public void doValidate(BaseProcessClass bpc) {
        ...
        String rfiComments = ParamUtil.getString(request,"rfiComments");
        String date = ParamUtil.getString(request,"dueDate");
        String status = ParamUtil.getString(request,"status");
        ...
        newAdhocRfiDto.setCommentsForApplicant(rfiComments);
        if(!StringUtils.isEmpty(date)&&CommonValidator.isDate(date)){
