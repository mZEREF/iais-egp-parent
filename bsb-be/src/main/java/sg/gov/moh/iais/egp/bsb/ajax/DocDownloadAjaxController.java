...
public class DocDownloadAjaxController {
    ...
    @GetMapping("/assessment-and-recommendation/repo/{id}")
    public void downloadAssessmentMohFile(@PathVariable("id") String maskedTmpId, HttpServletRequest request, HttpServletResponse response){
        downloadFile(request, response, maskedTmpId, this::unmaskFileId, this::getMohAssessmentInternalFile);
    }
    ...
    public MultipartFile incidentGetSavedFile(HttpServletRequest request, String id){
        ...
        DocDisplayDto dto = CollectionUtils.uniqueIndexMap(DocDisplayDto::getFileRepoId, docDisplayDtoList).get(id);
        if (dto == null) {
            throw new IllegalStateException(ERROR_MESSAGE_RECORD_INFO_NULL);
        }
        byte[] content = fileRepoClient.getFileFormDataBase(id).getEntity();
    ...
    public MultipartFile getMohAssessmentInternalFile(HttpServletRequest request, String id){
       List<DocRecordInfo> mohFile  = (List<DocRecordInfo>) ParamUtil.getSessionAttr(request, ModuleCommonConstants.KEY_INTERNAL_DOC_BSB_ASSESSMENT_AND_RECOMMENDATION);
       Map<String, DocRecordInfo> docRecordInfoMap = CollectionUtils.uniqueIndexMap(DocRecordInfo::getRepoId, mohFile);
       DocRecordInfo docRecordInfo = docRecordInfoMap.get(id);
