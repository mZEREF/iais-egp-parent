...
public class JaversUtil {
    ...
    public static void convert(String sumDiffMapKey, Diff diff, Map<String, Map<String, Object>> sumDiffMap) {
        // get change list
        List<ValueChange> valueChanges = diff.getChangesByType(ValueChange.class);
        if (!CollectionUtils.isEmpty(valueChanges)) {
            Map<String, Object> diffMap = new HashMap<>(valueChanges.size());
            for (ValueChange valueChange : valueChanges) {
                // key: field name
                diffMap.put(valueChange.getPropertyName(), valueChange.getRight());
            }
            sumDiffMap.put(sumDiffMapKey, diffMap);
        }
    }

    public static void convertList(String sumDiffMapKey, Diff diff, Map<String, Map<String, Object>> sumDiffMap) {
        // get change list
        List<ValueChange> valueChanges = diff.getChangesByType(ValueChange.class);
        if (!CollectionUtils.isEmpty(valueChanges)) {
            Map<String, Object> diffMap = new HashMap<>(valueChanges.size());
            for (ValueChange valueChange : valueChanges) {
                ...
                diffMap.put(diffMapKey, valueChange.getRight());
            }
            sumDiffMap.put(sumDiffMapKey, diffMap);
        }
    }
