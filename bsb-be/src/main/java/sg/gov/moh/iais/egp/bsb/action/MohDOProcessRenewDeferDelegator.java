...
@Delegator(value = "doProcessRenewDeferDelegator")
public class MohDOProcessRenewDeferDelegator {
    ...
    public void process(BaseProcessClass bpc) {
        ...
        switch (processingDecision) {
            ...
            case MOH_PROCESS_DECISION_REJECT:
                processClient.saveDOProcessRenewDeferApproveOrReject(appId, taskId, mohProcessDto);
...
//                rfiClient.saveDORecommendationRfi(mohProcessDto, appId, taskId);
                ParamUtil.setRequestAttr(request, ModuleCommonConstants.RESULT_MSG, "BISACKNEW004");
                break;
            default:
                log.error("don't have such processingDecision {}", StringUtils.normalizeSpace(processingDecision));
                break;
        }
    }
}