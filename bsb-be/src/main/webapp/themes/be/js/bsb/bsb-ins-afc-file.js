...
function uploadDoc() {
    ...
    if (validateUploadAFCDoc()) {
        var fileType = $("#fileType option:selected").val();
        $("input[name='docType']").val(fileType);
        $("input[name='action_type']").val("uploadDoc");
        // $("#cancelDoc").click();
        $("#mainForm").submit();
    }
    dismissWaiting();
}
function validateUploadAFCDoc() {
    var internalFileSelectedFile = $("#selectedFileInput");
    var filename = getAFCFileName(internalFileSelectedFile.val());
    ...
    var docType = $("#fileType option:selected").val();
    if (docType == null) {
...
function fileChange() {
    var file = $(this).val();
    if (file != null && file !== "") {
        $('#selectedFileShowTextName').val(getAFCFileName(file));
    } else {
        $('#selectedFileShowTextName').val("");
    }
}
