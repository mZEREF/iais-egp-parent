                                                    ...
                                                    <div class="col-xs-12">
                                                        <div class="table-gp">
                                                            <div class="form-horizontal">
                                                                ...
                                                                <div id="facValidityEndDateDiv" <c:if test="${mohProcessDto.processingDecision ne MasterCodeConstants.MOH_PROCESS_DECISION_APPROVE}">style="display: none;"</c:if>>
                                                                    <div class="form-group" <c:if test="${MasterCodeConstants.APP_TYPE_RFC ne mohProcessDto.submissionDetailsInfo.applicationType}">style="display: none;"</c:if>>
                                                                        <label class="col-xs-12 col-md-4 control-label">Facility Registration Validity End Date </label>
                                                                        <div class="col-sm-7 col-md-5 col-xs-10">
                                                                            <p><c:out value="${facilityDetailsInfo.validityEndDate}"/></p>
                                                                        </div>
                                                                    </div>
                                                                    <c:set var="validityEndDateTitle" value="Facility Registration Validity End Date"/>
                                                                    <c:if test="${MasterCodeConstants.APP_TYPE_RFC eq mohProcessDto.submissionDetailsInfo.applicationType}">
                                                                        <c:set var="validityEndDateTitle" value="New Facility Validity Date"/>
                                                                    </c:if>
                                                                    <c:set var="rfcNoInsAndCer" value="${MasterCodeConstants.APP_TYPE_RFC eq mohProcessDto.submissionDetailsInfo.applicationType
                                                                        && reportDto eq null && empty reviewAFCReportDto.certificationDocDisPlayDtos}"/>
                                                                    <div class="form-group" <c:if test="${rfcNoInsAndCer}">style="display: none;"</c:if>>
                                                                        <label for="facValidityEndDate" class="col-xs-12 col-md-4 control-label">${validityEndDateTitle} <span style="color: red">*</span></label>
                                                                        <div class="col-sm-7 col-md-5 col-xs-10">
                                                                            <c:if test="${MasterCodeConstants.MOH_PROCESS_FACILITY_PROCESS_TYPE.contains(mohProcessDto.submissionDetailsInfo.applicationSubType)}">
                                                                                <iais:datePicker id="facValidityEndDate" name="facValidityEndDate" value="${rfcNoInsAndCer?facilityDetailsInfo.validityEndDate:mohProcessDto.facValidityEndDate}"/>
                                                                                <span data-err-ind="facValidityEndDate" class="error-msg" ></span>
                                                                            </c:if>
                                                                            <c:if test="${MasterCodeConstants.MOH_PROCESS_BAT_PROCESS_TYPE.contains(mohProcessDto.submissionDetailsInfo.applicationSubType)}">
                                                                                <p><c:out value="${mohProcessDto.facValidityEndDate}"/></p>
                                                                            </c:if>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <c:set var="AOTitle" value="Select Approving Officer"/>
                                                                <c:if test="${MasterCodeConstants.APP_TYPE_RFC eq mohProcessDto.submissionDetailsInfo.applicationType}">
                                                                    <c:set var="AOTitle" value="Route to AO"/>
                                                                </c:if>
                                                                <div class="form-group" id="selectMohUserDiv" <c:if test="${mohProcessDto.processingDecision ne MasterCodeConstants.MOH_PROCESS_DECISION_APPROVE and mohProcessDto.processingDecision ne MasterCodeConstants.MOH_PROCESS_DECISION_REJECT}">style="display: none;"</c:if>>
                                                                    <label for="selectMohUser" class="col-xs-12 col-md-4 control-label">${AOTitle} <span style="color: red">*</span></label>
                                                                    <div class="col-sm-7 col-md-5 col-xs-10">
                                                                        <div class="input-group">
