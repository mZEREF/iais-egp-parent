<%@ taglib uri="http://java.sun.com/jstl/core_rt" prefix="c" %>
<%@ taglib prefix="iais" uri="http://www.ecq.com/iais" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<div class="row">
    <div class="alert alert-info" role="alert">
        <strong><h4>Inspection/Certification</h4></strong>
    </div>
    <div class="row">
        <div class="col-xs-12">
            <div class="table-gp">
                <div class="text">
                    <p><h4><strong><span>Inspection/Certification Checklist</span></strong></h4></p>
                </div>
                <table class="table table-bordered" style="margin-bottom: 0">
                    <thead>
                    <tr>
                        <th style="text-align:center;width:5%" align="center"><p>S/N</p></th>
                        <th style="text-align:center;"><p>Checklist</p></th>
                        <th style="text-align:center;"><p>Inspection Date</p></th>
                    </tr>
                    </thead>
                    <tbody style="text-align:center;">
                    <tr>
                        <td><p>1</p></td>
                        <td><p>Inspection Checklist</p></td>
                        <td><p>09 Jul 2021</p></td>
                    </tr>
                    <tr>
                        <td><p>2</p></td>
                        <td><p>Self-assessment Checklist</p></td>
                        <td><p>09 Jul 2021</p></td>
                    </tr>
                    <tr>
                        <td><p>3</p></td>
                        <td><p>Certification Checklist</p></td>
                        <td><p>09 Jul 2021</p></td>
                    </tr>
                    <tr>
                        <td><p>4</p></td>
                        <td><p>Audit Checklist</p></td>
                        <td><p>09 Jul 2021</p></td>
                    </tr>
                    </tbody>
                </table>
                <div>&nbsp</div>
                <div class="text">
                    <p><h4><strong><span>Inspection/Certification Findings</span></strong></h4></p>
                </div>
                <table class="table table-bordered" style="margin-bottom: 0">
                    <thead>
                    <tr>
                        <th style="text-align:center;width:5%" align="center"><p></p>S/N</th>
                        <th style="text-align:center;"><p>Checklist Question</p></th>
                        <th style="text-align:center;"><p>Type</p></th>
                        <th style="text-align:center;"><p>Deadline</p></th>
                        <th style="text-align:center;"><p>Rectified</p></th>
                    </tr>
                    </thead>
                    <tbody style="text-align:center;">
                    <tr>
                        <td><p>1</p></td>
                        <td><p>Question 1</p></td>
                        <td><p>Follow Up</p></td>
                        <td><p>08 Jul 2021</p></td>
                        <td><p>No</p></td>
                    </tr>
                    <tr>
                        <td><p>2</p></td>
                        <td><p>Question 2</p></td>
                        <td><p>Non-Compliance</p></td>
                        <td><p>-</p></td>
                        <td><p>Yes</p></td>
                    </tr>
                    </tbody>
                </table>
                <div>&nbsp</div>
                <div class="text">
                    <p><h4><strong><span>Inspection/Certification Report and Documents</span></strong></h4></p>
                </div>
                <table class="table table-bordered" style="margin-bottom: 0">
                    <thead>
                    <tr>
                        <th style="text-align:center;width:5%" align="center"><p>S/N</p></th>
                        <th style="text-align:center;"><p>File Name</p></th>
                    </tr>
                    </thead>
                    <tbody style="text-align:center;">
                    <tr>
                        <td><p>1</p></td>
                        <td><p>Inspection Report</p></td>
                    </tr>
                    <tr>
                        <td><p>2</p></td>
                        <td><p>Certification Report</p></td>
                    </tr>
                    <tr>
                        <td><p>3</p></td>
                        <td><p>Certification Doc uploaded by AFC</p></td>
                    </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>

